...
public class TransporteResourceIT {
     ...
     *
     * @see com.bussapp.web.repository.search.TransporteSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final TransporteResource transporteResource = new TransporteResource(transporteRepository, mockTransporteSearchRepository);
    ...
    public void createTransporte() throws Exception {
        ...
        // Validate the Transporte in Elasticsearch
        verify(mockTransporteSearchRepository, times(1)).save(testTransporte);
    }
    ...
    public void createTransporteWithExistingId() throws Exception {
        ...
        // Validate the Transporte in Elasticsearch
        verify(mockTransporteSearchRepository, times(0)).save(transporte);
    }
    ...
    public void deleteTransporte() throws Exception {
        ...
        // Validate the Transporte in Elasticsearch
        verify(mockTransporteSearchRepository, times(1)).deleteById(transporte.getId());
    }

    @Test
    @Transactional
    public void searchTransporte() throws Exception {
        // Initialize the database
        transporteRepository.saveAndFlush(transporte);
        when(mockTransporteSearchRepository.search(queryStringQuery("id:" + transporte.getId())))
            .thenReturn(Collections.singletonList(transporte));
        // Search the transporte
        restTransporteMockMvc.perform(get("/api/_search/transportes?query=id:" + transporte.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(transporte.getId().intValue())))
            .andExpect(jsonPath("$.[*].lugares").value(hasItem(DEFAULT_LUGARES)))
