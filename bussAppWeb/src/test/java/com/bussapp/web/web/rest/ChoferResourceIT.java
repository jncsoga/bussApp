...
public class ChoferResourceIT {
     ...
     *
     * @see com.bussapp.web.repository.search.ChoferSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final ChoferResource choferResource = new ChoferResource(choferRepository, mockChoferSearchRepository);
    ...
    public void createChofer() throws Exception {
        ...
        // Validate the Chofer in Elasticsearch
        verify(mockChoferSearchRepository, times(1)).save(testChofer);
    }
    ...
    public void createChoferWithExistingId() throws Exception {
        ...
        // Validate the Chofer in Elasticsearch
        verify(mockChoferSearchRepository, times(0)).save(chofer);
    }
    ...
    public void deleteChofer() throws Exception {
        ...
        // Validate the Chofer in Elasticsearch
        verify(mockChoferSearchRepository, times(1)).deleteById(chofer.getId());
    }

    @Test
    @Transactional
    public void searchChofer() throws Exception {
        // Initialize the database
        choferRepository.saveAndFlush(chofer);
        when(mockChoferSearchRepository.search(queryStringQuery("id:" + chofer.getId())))
            .thenReturn(Collections.singletonList(chofer));
        // Search the chofer
        restChoferMockMvc.perform(get("/api/_search/chofers?query=id:" + chofer.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(chofer.getId().intValue())))
            .andExpect(jsonPath("$.[*].usuario").value(hasItem(DEFAULT_USUARIO)));
