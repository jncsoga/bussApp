...
public class RutaResource {
    ...
    private final RutaSearchRepository rutaSearchRepository;

    public RutaResource(RutaRepository rutaRepository, RutaSearchRepository rutaSearchRepository) {
        this.rutaRepository = rutaRepository;
        this.rutaSearchRepository = rutaSearchRepository;
    }
    ...
    public ResponseEntity<Ruta> createRuta(@RequestBody Ruta ruta) throws URISyntaxException {
        ...
        rutaSearchRepository.save(result);
    ...
    public ResponseEntity<Ruta> updateRuta(@RequestBody Ruta ruta) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, ruta.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Void> deleteRuta(@PathVariable Long id) {
        ...
        rutaSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/rutas?query=:query} : search for the ruta corresponding
     * to the query.
     *
     * @param query the query of the ruta search.
     * @return the result of the search.
     */
    @GetMapping("/_search/rutas")
    public List<Ruta> searchRutas(@RequestParam String query) {
        log.debug("REST request to search Rutas for query {}", query);
        return StreamSupport
            .stream(rutaSearchRepository.search(queryStringQuery(query)).spliterator(), false)
