...
import com.bussapp.web.repository.search.TransporteSearchRepository;
...
public class TransporteResource {
    ...
    private final TransporteSearchRepository transporteSearchRepository;

    public TransporteResource(TransporteRepository transporteRepository, TransporteSearchRepository transporteSearchRepository) {
        this.transporteRepository = transporteRepository;
        this.transporteSearchRepository = transporteSearchRepository;
    }
    ...
    public ResponseEntity<Transporte> createTransporte(@RequestBody Transporte transporte) throws URISyntaxException {
        ...
        transporteSearchRepository.save(result);
    ...
    public ResponseEntity<Transporte> updateTransporte(@RequestBody Transporte transporte) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, transporte.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Void> deleteTransporte(@PathVariable Long id) {
        ...
        transporteSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/transportes?query=:query} : search for the transporte corresponding
     * to the query.
     *
     * @param query the query of the transporte search.
     * @return the result of the search.
     */
    @GetMapping("/_search/transportes")
    public List<Transporte> searchTransportes(@RequestParam String query) {
        log.debug("REST request to search Transportes for query {}", query);
        return StreamSupport
            .stream(transporteSearchRepository.search(queryStringQuery(query)).spliterator(), false)
